...
reportAnonymised  <- isTRUE(params$report_wait_anonymised)
...
if (!("All Organs" %in% report_organs)) {
  rxWaitReportData <- rxWaitReportData |>
    dplyr::filter(Organ %in% report_organs)
}
...
# Colour the TCI data accordingly so we can see 'expired TCIs' easily in red
date_cols <- names(rxWaitReportData)[vapply(rxWaitReportData, inherits, logical(1), "Date")]

rxWaitReportData <- rxWaitReportData |>
  mutate(
    TciDate_raw = TciDate,
    across(
      .cols = setdiff(date_cols, "TciDate"),
      .fns = function(x) {
        if (reportAnonymised) {
          anonymise_date(x)
        } else {
    ...
    TciDate = cell_spec(
      if (reportAnonymised)
        anonymise_date(TciDate_raw)
      else
        format(TciDate_raw, "%d-%m-%Y"),
      color = ifelse(
        is.na(TciDate_raw), "black",
        ifelse(TciDate_raw < Sys.Date(), "#90EE90", "black")
      ),
  ...
  rxWaitReportData <- rxWaitReportData %>% arrange(ID)
...
if (nrow(rxWaitReportData) > 0) {
  if (reportAnonymised) if ("ID" %in% names(rxWaitReportData)) rxWaitReportData$ID <- anonymise_ptid(rxWaitReportData$ID)
  options(knitr.kable.NA = "")
  kbl(
    rxWaitReportData,
    caption = paste("Current Waiting List (Created ", generatedTime, ")", sep = ""),
    escape = FALSE
  ) %>%
    kable_styling() %>%
...
Notes on TCI Table:
<ul>
  <li>TCI dates in <span style="color: #90EE90">green</span> have expired, please check/update source data treatment date.</li>
...
{
  if (reportAnonymised) if ("ID" %in% names(rxClockStopData)) rxClockStopData$ID <- anonymise_ptid(rxClockStopData$ID)
...
```{r tci_calendar_key_html, echo=FALSE, eval = !knitr::is_latex_output()}
# HTML branch: function returns a tags object; knitr will render it
# returns tag object (let knitr render it) Note DONT USE ASIS on the HTML Chunk**
getTciCalendarKey()
```

```{r tci_calendar_key_latex, echo=FALSE, results='asis', eval = knitr::is_latex_output()}
# LaTeX branch: function must return knitr::asis_output(latex)
getTciCalendarKey()
```
...
if (isChromeInstalled())
{
  ...
  # Create two widgets
  cal_this_month <- makeTciCalendar(reportStart, tciThisMonth, report_organs, TRUE, FALSE, anonymised = reportAnonymised)
  cal_next_month <- makeTciCalendar(reportStart, tciNextMonth, report_organs, TRUE, FALSE, anonymised = reportAnonymised)
  ...
  page_this <- tagList(
    tags$head(
      tags$style(HTML("html,body{margin:0;padding:0;} #calendar-wrapper-this{height:1000px !important;} #calendar-wrapper-this .htmlwidget{height:100% !important;}"))),
    tags$div(id = "calendar-wrapper-this", cal_this_month)
  )
  page_next <- tagList(
    tags$head(
      tags$style(HTML("html,body{margin:0;padding:0;} #calendar-wrapper-next{height:1000px !important;} #calendar-wrapper-next .htmlwidget{height:100% !important;}"))),
  ...
  {
    cat("\\clearpage\n")
    cat(sprintf("\\section*{TCI Calendar: %s}\n", format(tciThisMonth, "%b %Y")))
    cat("\\begin{center}\n")
    cat(sprintf("\\includegraphics[width=\\textwidth]{%s}\n",
                normalizePath(png_this, winslash = "/")))
    cat("\\end{center}\n")
  
    cat("\\clearpage\n")
    cat(sprintf("\\section*{TCI Calendar: %s}\n", format(tciNextMonth, "%b %Y")))
    cat("\\begin{center}\n")
    cat(sprintf("\\includegraphics[width=\\textwidth]{%s}\n",
                normalizePath(png_next, winslash = "/")))
    cat("\\end{center}\n")
  
  
  # show key and embed PNGs (works for PDF and HTML)
  #if (knitr::is_latex_output()) {
  #  cat(sprintf("## TCI Calendar: %s\n\n", format(tciThisMonth, "%b %Y")))
  #  knitr::include_graphics(normalizePath(png_this))
  #  cat("\n\n")
  #  cat(sprintf("## TCI Calendar: %s\n\n", format(tciNextMonth, "%b %Y")))
  #  knitr::include_graphics(normalizePath(png_next))
  #}
  #if (knitr::is_latex_output()) {
  ...
  #  print(knitr::include_graphics(png_next))
    ...
    cat(sprintf("<h3>TCI Calendar for %s</h3>", format(tciThisMonth, "%b %Y")))
    cat(sprintf(
      '<img src="file://%s" style="width:100%%; height:auto; border:1px solid #ddd; margin-bottom:20px;" />',
      normalizePath(png_this)
    ))
    cat(sprintf("<h3>TCI Calendar for %s</h3>", format(tciNextMonth, "%b %Y")))
    cat(sprintf(
      '<img src="file://%s" style="width:100%%; height:auto; border:1px solid #ddd; margin-bottom:20px;" />',
...
makeCalendarHeatmap(report_start_date, report_organs, TRUE, anonymised = reportAnonymised)
```

```{r tciCalendarNextYear, echo = FALSE, warning = FALSE}
makeCalendarHeatmap(as.Date(report_start_date) + 365, report_organs, TRUE, anonymised = reportAnonymised)
```
...
```{r reportLogo, echo = FALSE, warning = FALSE}
# copy logo into the report output dir so it's always available during rendering
# The RMD knits in a temp directory (as we can see if we run getwd() here), so we need to use here library to get actual path from which we can find the logo
