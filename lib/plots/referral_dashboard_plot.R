...
refToDTTMeanPlot <- function(referralTimes, range_by = "Monthly") {
  referralTimes |>
    mutate(BreachStatus = if_else(MeanRefToDTT > 10, if_else(MeanRefToDTT > 31, ">31 days", ">10 days"), "<10 days")) |>
    ggplot() +
    ...
    geom_hline(yintercept = 31, linetype = "dashed", color = "red") +
    scale_color_manual(
      name = "Breach Status",
      values = c("<10 days" = "grey", ">10 days" = "orange", ">31 days" = "red")
    ) +
    guides(color = "none") +
...
refToDTTCountPlot <- function(referralTimes, range_by = "Monthly") {
  referralTimes |>
    ggplot() +
    geom_bar(aes(x = PeriodStart, y = RefToDTTStopped, fill = "Stopped"),
      stat = "identity",
      alpha = 0.75,
      just = 0,
      position = "dodge2"
    ) +
    geom_bar(aes(x = PeriodStart, y = RefToDTTNotStopped, fill = "Not Stopped"),
...
dttToRxMeanPlot <- function(referralTimes, range_by = "Monthly") {
  referralTimes |>
    mutate(BreachStatus = if_else(MeanDTTToRx > 31, if_else(MeanDTTToRx > 45, ">45 days", ">31 days"), "<31 days")) |>
    ggplot() +
    ...
    scale_color_manual(
      name = "Breach Status",
      values = c("<31 days" = "grey", ">31 days" = "orange", ">45 days" = "red")
    ) +
    guides(color = "none") +
    scale_x_date(date_breaks = "month", date_labels = "%b-%y") +
    labs(
